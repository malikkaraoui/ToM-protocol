import Foundation
import TomProtocolFFI
import os.log

/// Wrapper acteur au-dessus de la FFI C de tom-protocol.
///
/// Sérialise tous les accès au handle natif. Modèle d'erreur : les fonctions
/// C renvoient `-1` + détail via `tom_node_last_error`, traduit en `TomError`.
public actor TomNodeWrapper {
    private let log = Logger(subsystem: "org.tom-protocol.tom-node", category: "TomNodeWrapper")
    private var handle: OpaquePointer?

    public init() {}

    private func normalizedOptionalString(_ value: String?) -> String? {
        guard let value else { return nil }
        let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
        return trimmed.isEmpty ? nil : trimmed
    }

    public var isRunning: Bool {
        handle != nil
    }

    public func create(relayUrl: String?, identityPath: String?, n0Discovery: Bool) throws {
        guard handle == nil else {
            throw TomError.alreadyRunning
        }

        var config: [String: Any] = [
            "n0_discovery": n0Discovery
        ]
        if let relayUrl = normalizedOptionalString(relayUrl) {
            config["relay_url"] = relayUrl
        }
        if let path = identityPath {
            config["identity_path"] = path
        }

        let jsonData = try JSONSerialization.data(withJSONObject: config)
        guard let jsonStr = String(data: jsonData, encoding: .utf8) else {
            throw TomError.jsonParseFailed("Failed to encode config")
        }

        log.info("Creating node with config: \(jsonStr)")

        guard let h = jsonStr.withCString({ tom_node_create($0) }) else {
            throw TomError.ffiReturnedNull(function: "tom_node_create")
        }

        handle = h
        log.info("Node created successfully")
    }

    public func start(
        username: String,
        encryption: Bool,
        enableDht: Bool,
        relayUrl: String?,
        identityPath: String?,
        n0Discovery: Bool,
        localDiscovery: Bool,
        dataDir: String?,
        gossipBootstrapPeers: [String] = []
    ) throws {
        guard let h = handle else {
            throw TomError.notStarted
        }

        var config: [String: Any] = [
            "username": username,
            "encryption": encryption,
            "enable_dht": enableDht,
            "n0_discovery": n0Discovery,
            "local_discovery": localDiscovery
        ]
        if let relayUrl = normalizedOptionalString(relayUrl) {
            config["relay_url"] = relayUrl
        }
        if let path = identityPath {
            config["identity_path"] = path
        }
        if let dir = dataDir {
            config["data_dir"] = dir
        }
        let normalizedBootstrapPeers = gossipBootstrapPeers
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }
        if !normalizedBootstrapPeers.isEmpty {
            config["gossip_bootstrap_peers"] = normalizedBootstrapPeers
        }

        let jsonData = try JSONSerialization.data(withJSONObject: config)
        guard let jsonStr = String(data: jsonData, encoding: .utf8) else {
            throw TomError.jsonParseFailed("Failed to encode runtime config")
        }

        log.info("Starting node runtime...")

        let result = jsonStr.withCString { tom_node_start(h, $0) }
        if result != 0 {
            var errorDetail = "tom_node_start returned \(result)"
            if let errPtr = tom_node_last_error(h) {
                errorDetail = String(cString: errPtr)
                tom_node_free_string(errPtr)
            }
            log.error("Start failed: \(errorDetail)")
            throw TomError.unknown(errorDetail)
        }

        log.info("Node runtime started")
    }

    public func stop() {
        guard let h = handle else { return }
        log.info("Stopping node...")
        tom_node_stop(h)
        handle = nil
        log.info("Node stopped")
    }

    /// Drop the handle without graceful shutdown (safe after OS suspend)
    public func forceReset() {
        guard let h = handle else { return }
        log.warning("Force-resetting node handle (post-sleep cleanup)")
        tom_node_free(h)
        handle = nil
    }

    public func sendMessage(to target: NodeId, payload: Data) throws {
        guard let h = handle else {
            throw TomError.notStarted
        }

        let result = target.withCString { targetCStr in
            payload.withUnsafeBytes { bufPtr -> Int32 in
                let ptr = bufPtr.baseAddress?.assumingMemoryBound(to: UInt8.self)
                return tom_node_send_message(h, targetCStr, ptr, UInt(bufPtr.count))
            }
        }

        if result != 0 {
            throw TomError.sendFailed("tom_node_send_message returned \(result)")
        }
    }

    public func createGroup(name: String, members: [NodeId], inviteOnly: Bool) throws {
        guard let h = handle else {
            throw TomError.notStarted
        }

        let config: [String: Any] = [
            "name": name,
            "initial_members": members,
            "invite_only": inviteOnly
        ]

        let jsonData = try JSONSerialization.data(withJSONObject: config)
        guard let jsonStr = String(data: jsonData, encoding: .utf8) else {
            throw TomError.jsonParseFailed("Failed to encode group config")
        }

        let result = jsonStr.withCString { tom_node_create_group(h, $0) }
        if result != 0 {
            throw TomError.groupCreateFailed("tom_node_create_group returned \(result)")
        }
    }

    public func sendGroupMessage(groupId: GroupId, text: String) throws {
        guard let h = handle else {
            throw TomError.notStarted
        }

        let result = groupId.withCString { gidCStr in
            text.withCString { textCStr in
                tom_node_send_group_message(h, gidCStr, textCStr)
            }
        }

        if result != 0 {
            throw TomError.sendFailed("tom_node_send_group_message returned \(result)")
        }
    }

    public func addPeerAddr(nodeId: NodeId, relayUrl: String? = nil, directAddrs: [String]? = nil) throws {
        guard let h = handle else {
            throw TomError.notStarted
        }

        var config: [String: Any] = ["node_id": nodeId]
        if let relay = normalizedOptionalString(relayUrl) { config["relay_url"] = relay }
        if let addrs = directAddrs { config["direct_addrs"] = addrs }

        let jsonData = try JSONSerialization.data(withJSONObject: config)
        guard let jsonStr = String(data: jsonData, encoding: .utf8) else {
            throw TomError.jsonParseFailed("Failed to encode peer addr")
        }

        let result = jsonStr.withCString { tom_node_add_peer_addr(h, $0) }
        if result != 0 {
            throw TomError.sendFailed("tom_node_add_peer_addr returned \(result)")
        }
    }

    public func connectedPeers() -> [NodeId] {
        guard let h = handle else { return [] }

        guard let cStr = tom_node_connected_peers(h) else { return [] }
        let jsonStr = String(cString: cStr)
        tom_node_free_string(cStr)

        guard let data = jsonStr.data(using: .utf8) else { return [] }

        do {
            return try JSONDecoder().decode([NodeId].self, from: data)
        } catch {
            log.error("Failed to decode peers: \(error.localizedDescription)")
            return []
        }
    }

    public func discoveredPeers() -> [TomPeer] {
        guard let h = handle else { return [] }

        guard let cStr = tom_node_discovered_peers(h) else { return [] }
        let jsonStr = String(cString: cStr)
        tom_node_free_string(cStr)

        guard jsonStr != "[]",
              let data = jsonStr.data(using: .utf8) else {
            return []
        }

        do {
            return try JSONDecoder().decode([TomPeer].self, from: data)
        } catch {
            log.error("Failed to decode discovered peers: \(error.localizedDescription)")
            return []
        }
    }

    public func receiveMessages() -> [TomMessage] {
        guard let h = handle else { return [] }

        guard let cStr = tom_node_receive_messages(h) else { return [] }
        let jsonStr = String(cString: cStr)
        tom_node_free_string(cStr)

        guard jsonStr != "[]",
              let data = jsonStr.data(using: .utf8) else {
            return []
        }

        do {
            return try JSONDecoder().decode([TomMessage].self, from: data)
        } catch {
            log.error("Failed to decode messages: \(error.localizedDescription)")
            return []
        }
    }

    /// Returns the best available relay URL: configured relay first, then gossip-discovered.
    /// Returns nil when no relay is known yet.
    public func getDiscoveredRelay() -> String? {
        guard let h = handle else { return nil }
        guard let cStr = tom_get_discovered_relay(h) else { return nil }
        defer { tom_node_free_string(cStr) }
        let url = String(cString: cStr)
        return url.isEmpty ? nil : url
    }

    public func status() -> TomNodeStatus? {
        guard let h = handle else { return nil }

        guard let cStr = tom_node_status(h) else { return nil }
        let jsonStr = String(cString: cStr)
        tom_node_free_string(cStr)

        guard let data = jsonStr.data(using: .utf8) else { return nil }

        do {
            return try JSONDecoder().decode(TomNodeStatus.self, from: data)
        } catch {
            log.error("Failed to decode status: \(error.localizedDescription)")
            return nil
        }
    }
}
