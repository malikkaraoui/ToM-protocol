...
impl From<AccessConfig> for tom_relay::server::AccessConfig {
    fn from(cfg: AccessConfig) -> Self {
        match cfg {
            AccessConfig::Everyone => tom_relay::server::AccessConfig::Everyone,
            AccessConfig::Allowlist(allow_list) => {
                let allow_list = Arc::new(allow_list);
                tom_relay::server::AccessConfig::Restricted(Box::new(move |endpoint_id| {
                    let allow_list = allow_list.clone();
                    async move {
                        if allow_list.contains(&endpoint_id) {
                            tom_relay::server::Access::Allow
                        } else {
                            tom_relay::server::Access::Deny
                        }
                    }
                    .boxed()
                }))
...
) -> tom_relay::server::Access {
    use tom_relay::server::Access;
...
struct TlsConfig {
    ...
    ///
    /// Defaults to the `https_bind_addr` with the port set to [`tom_relay::defaults::DEFAULT_RELAY_QUIC_PORT`].
    ///
    /// If `https_bind_addr` is not set, defaults to `http_bind_addr` with the
    /// port set to [`tom_relay::defaults::DEFAULT_RELAY_QUIC_PORT`]
