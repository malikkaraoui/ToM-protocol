...
/// Runs a relay server.
///
/// If `quic` is set to `true`, it will make the appropriate [`QuicConfig`] from the
/// generated tls certificates and run the quic server at a random free port.
///
...
pub async fn run_relay_server_with(quic: bool) -> Result<(RelayMap, RelayUrl, Server), SpawnError> {
    let (certs, server_config) = tom_relay::server::testing::self_signed_tls_certs_and_config();
...
pub(crate) mod dns_and_pkarr_servers {
    ...
    impl DnsPkarrServer {
        ...
        pub async fn run() -> std::io::Result<Self> {
            Self::run_with_origin("dns.tom.test".to_string()).await
        }
        ...
        /// Create a [`ConcurrentAddressLookup`] with DNS and Pkarr configured for test servers.
        pub fn address_lookup(
            &self,
            secret_key: tom_base::SecretKey,
...
pub(crate) mod dns_server {
    ...
    /// Run a DNS server. Must pass a [`QueryHandler`] that answers queries.
...
pub(crate) mod pkarr_dns_state {
    ...
    use tom_base::EndpointId;
    use tom_relay::endpoint_info::{EndpointIdExt, EndpointInfo, TOM_TXT_NAME};
    ...
    /// Converts a [`EndpointInfo`] into hickory DNS records.
    ...
    /// Converts to a list of hickory resource records.
    fn to_hickory_records(
        ...
        let name = format!("{TOM_TXT_NAME}.{}.{origin}", endpoint_id.to_z32());
