...
| 2026-07-17 | Cycle de vie mobile | **Modèle background iOS/tvOS = grâce courte puis arrêt PROPRE + réveils opportunistes** (implémentation du non-objectif 2026-07-15 « pas de daemon ») : `beginBackgroundTask` ~18 s (retour court = zéro restart), quiesce propre au-delà (assertion tenue ~8 s pour couvrir le teardown Rust détaché), `UIBackgroundModes [fetch]` + BGAppRefreshTask (~30 s de relève backup, replanifié à chaque background) — **PAS de mode audio/voip/location** (daemon déguisé, rejeté) | Les pairs voient une déconnexion NETTE (fin du zombie/flapping — 0 échec flotte pendant absence, vs 187-425/appareil) ; le backup ADR-009 est LE filet pendant l'absence ; app visible dans Réglages > Actualisation en arrière-plan | Commits `8a4eeb2`+`09757ba` (build 98), validation terrain 2026-07-17, mémoire `ios-background-lifecycle-48` |
| 2026-07-17 | Contrat FFI→Swift | **Tout modèle Swift décodé depuis le JSON FFI doit avoir un `init(from:)` EXPLICITE + un test de contrat décodant le JSON wire EXACT** (TomProtocolKitTests) — plus jamais de décodage synthétisé sur la frontière FFI | Un champ non-Optional ajouté côté Swift sans init explicite jette TOUT le batch en silence (`keyNotFound` → `catch { return [] }`) — cause des builds 86-95 aveugles (messages_recus=0, 9h de fausses pistes transport) | Commit `a90a889` (build 96), mémoire `swift-codable-ffi-decode-contract` |
