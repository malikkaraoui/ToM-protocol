...
IOS_PROJ=apps/tom-node-ios/TomNode.xcodeproj
TV_PROJ=apps/tom-node-tvos/TomNode.xcodeproj
...
deploy_ios() {
  ...
  xcodebuild -project "$IOS_PROJ" -scheme TomNode -configuration Debug \
...
deploy_tvos() {
  ...
  log "[build] tvOS ..."
  xcodebuild -project "$TV_PROJ" -scheme TomNode -configuration Debug \
