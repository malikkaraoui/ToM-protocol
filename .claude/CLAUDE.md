...
| Phase | **build 139** (flotte uniforme). **CAP = ORGANISME À RÔLES** (recadrage Malik 20/07). **Session 21/07 soir — POUSSÉ** : herméticité étage L (`237b9fe` : `TomNodeConfig::hermetic()` + `GatedHandler`/`set_allowed_peers`, oracle zéro-étranger, 23/23 roles-charge) ; R4-F rendez-vous multi-host (`2cbf964` : sous-cmd `tom-stress r4`, baseline froide RÉELLE ~25 s Mac↔NAS). **NON poussé (WIP)** : flag `--production-rendezvous` (main.rs+scenario_r4), `scripts/chaos/capstone.py`, docs (`capstone-run-checklist`, `PROCHAINE-SESSION-carnet-rendezvous`, `PROMPT-fix-freebox-oom`). **Capstone Phase C** : R4-F au capstone VERT (inconnu vu par la flotte) mais V1/backup/spam INVALIDES (NAS déjà malade). **GROS SUJET OUVERT — OOM Freebox autopsié** : fuite RÉELLE (4 OOM), ∝ interaction nœuds (test 8-nœuds : 38→229 Mo, ne redescend pas) ; **transport INNOCENTÉ** (repro LEAKHUB), coupable = **self-relay OU DHT sous interaction** (call-site non tranché) ; 2 couches → bug mémoire + conception carnet diffusif. NAS restauré propre (30 Mo). **Findings antérieurs ouverts** : KL#13 (famine spoof), KL#14-raffinement, flake CI DHT Testnet. **Reprise** : 2 prompts prêts — `docs/plans/PROMPT-fix-freebox-oom.md` (chasse bug + carnet borné ~30 contacts, multi-agent) et `docs/plans/PROCHAINE-SESSION-carnet-rendezvous.md` (rôle rendez-vous tournant, sans point fixe). Mémoires : `tom-freebox-oom-carnet-rendezvous`, `session-handoff-2026-07-21-soir`. |
