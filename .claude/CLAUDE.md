...
| Phase | **build 137** (2026-07-20 soir), flotte UNIFORME 137 (Mac + iPad + 2 iPhones dont Laura, NAS ; ATV retirée) — re-probe 15s ENFIN en terrain. LIVRÉS cette session : Lot C tuné 15s (135) ; fix export UDP headless→collecteur (136, SO_BROADCAST + unicast NAS) ; collecteur versionné `scripts/chaos/tom_collector.py` ; audit instrumentation Phase 0 (`docs/plans/audit-instrumentation-phase0.md`, 9 findings — compteurs = ENVELOPES pas messages) ; charte v1 recadrée + banc courbe in-process (2 commits LOCAUX non poussés : doc+oracle+gate à finir). **INCIDENT assumé** : le banc a pollué le rendez-vous DHT partagé (~150 fantômes, `enable_dht:true` par défaut) — corrigé + flotte purgée. **RECADRAGE MALIK (le cap)** : le réseau = ORGANISME À RÔLES (L1, subnets, relais, backup, observateurs, validation, carnet de rendez-vous TOURNANT) — pas un tuyau send/ACK ; relire TOUTES les notes via QMD et incorporer les rôles aux scénarios. Prompt de reprise : `docs/plans/PROMPT-REPRISE-ROLES.md`. UX à traiter : boutons reset inertes (haptique+visuel), auto-start 2s noté. |
