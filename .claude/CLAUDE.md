...
| Phase | **build 139** (2026-07-21), flotte UNIFORME 139 (Mac + iPad + ATV réintégrée + 2 iPhones dont Laura + NAS). **CAP = ORGANISME À RÔLES** (recadrage Malik 20/07, verbatim dans `docs/plans/PROMPT-REPRISE-ROLES.md`) : le réseau n'est PAS un tuyau send/ACK — L1, subnets, relais, backup, observateurs, validation, carnet de rendez-vous TOURNANT. **LIVRÉ cette session** (le mandat matérialisé) : (1) relecture intégrale des notes → `docs/plans/prisme-des-roles.md` (grille des 9 rôles vision→code→écart, scénarios R1-R8) ; (2) banc courbe finalisé/poussé (intégrité seule, 3 artefacts de mesure tués) ; (3) **banc `tom-stress roles-charge` Phase A COMPLÈTE — 22 oracles PASS** (r1 multi-hop+promotion, r2 backup absent→retour, r5 subnets, r7 PoP anti-fantômes, r8 arroseur) ; (4) **3 BUGS PROTOCOLAIRES débusqués par le banc et corrigés** : antispam percé (`e1dc778`), crédit-origine au relais (`2937330`), backup non purgé à l'ACK (`241b447`, KL#14) ; (5) P1 namespace rendez-vous isolé + P3 timeout subnet (débloquent R4/R5) ; (6) 2 API observabilité (`get_peer_statuses`, `get_backup_pending_count`) ; (7) UX reset (spinner+toast+haptique) + compte à rebours auto-start 5s. **Findings ouverts** : KL#13 (famine par spoof, ordre antispam→signature), KL#14-raffinement (broadcast multi-replica), fuite herméticité ENTRANTE du banc (flotte locale via mDNS → Known ; élucide le « chatter » du banc courbe), flake CI `rendezvous_recovery_after_total_blackout` (Testnet). **Reprise** : `docs/plans/PROMPT-REPRISE-ROLES.md` + mémoire `session-handoff-2026-07-21`. Prochain gros chantier : capstone R4-F multi-host (organisme sous charge). |
